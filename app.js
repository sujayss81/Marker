...
app.post("/studentLogin", function (req, res, next) {
  ...
  dbs
    .collection("students")
    .findOne({ usn: usn, pass: pass }, function (err, result) {
      if (err) next(e);
      if (result == null) {
        res.status(404);
        // res.end();
...
app.post("/teacherLogin", function (req, res) {
  ...
  dbs.collection("teachers").findOne(obj, function (err, result) {
    ...
    if (result == null) {
      ...
      dbs
        .collection("subjects")
        .findOne({ sub_id: result.sub_id }, function (e, r) {
          if (e) next(e);
          if (r == null) {
            res.status(500);
            res.end();
...
app.post("/changePassword", function (req, res, next) {
  ...
  dbs
    .collection("teachers")
    .findOne({ e_id: eid, pass: oldpassword }, function (e, r) {
      if (e) console.log(e);
      if (r == null) {
        res.status(404);
        console.log("hello");
        ...
        dbs
          .collection("teachers")
          .updateOne(r, newvalues, function (err, result) {
            if (err) {
              res.status(500);
              res.end();
              next(err);
...
app.post("/cPassword", function (req, res, next) {
  ...
  dbs
    .collection("students")
    .findOne({ usn: usn, pass: oldpassword }, function (e, r) {
      if (e) console.log(e);
      if (r == null) {
        res.status(404);
        console.log("hello");
...
app.get("/generateQR/:subid", function (req, res) {
  dbs
    .collection("subjects")
    .findOne({ sub_id: req.params.subid }, function (e, r) {
      if (e) {
        console.log(e);
      } else {
        if (r == null) {
      ...
      dbs.collection("students").findOne({ usn: stud_id }, function (e, r) {
        ...
        if (r == null) {
          ...
          dbs
            .collection(date)
            .countDocuments(
              { usn: stud_id, stud_name: r.name, sub_id: sub_id },
              function (e, resu) {
                if (e) console.log(e);
                console.log(resu);
                if (resu == 0) {
                  dbs
                    .collection(date)
                    .insertOne(
                      { usn: stud_id, stud_name: name, sub_id: sub_id },
                      function (e, result) {
                        if (e) console.log(e);
